...
# Reutilizamos la base vectorial del TFG que ya creamos
...
# --- Funciones reales que ejecutan cada herramienta ---
...
def calculadora(expresion):
    try:
        # eval es peligroso en general, pero aquí es un ejercicio de aprendizaje controlado
...
# Mapeamos nombre de herramienta -> función real
...
def preguntar_al_agente(pregunta_usuario):
    ...
    while True:
        ...
        # Si Claude ya no necesita más herramientas, devolvemos su respuesta final
        if respuesta.stop_reason != "tool_use":
            texto_final = next((b.text for b in respuesta.content if b.type == "text"), "")
            return texto_final

        # Si necesita usar herramientas, las ejecutamos todas las que pida
        historial.append({"role": "assistant", "content": respuesta.content})

        resultados_herramientas = []
        for bloque in respuesta.content:
            if bloque.type == "tool_use":
                print(f"  [Usando herramienta: {bloque.name} con {bloque.input}]")
        ...
        # El bucle vuelve a empezar: Claude ve el resultado y decide si necesita algo más
